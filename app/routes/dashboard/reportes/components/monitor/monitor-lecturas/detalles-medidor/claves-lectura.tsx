...
export default function ClavesLectura({
  ...
  // ✅ REFACTOR: Verificamos todas las entradas del array, no solo la primera
  const hasClaveCritica = data.some(
    item =>
      item.CLA_Tipo === 3 && // Tipo 3 indica clave crítica/alerta
      item.CLA_Codigo !== 'LEOK' // Excluir explícitamente LEOK por seguridad
  );

  // ✅ REFACTOR: Extraer lógica de renderizado condicional
  const renderContent = () => {
    if (error) {
    ...
    return (
      <div className="space-y-2 sm:space-y-3">
        {data.map(item => (
          <div
            key={`${item.CLA_Codigo}-${item.CLL_Fecha}`}
            className="p-3 sm:p-4 bg-muted/30 rounded-xl border border-border/20 hover:bg-muted/50 transition-colors"
          >
            <div className="flex flex-col sm:flex-row sm:items-start gap-2 sm:gap-4">
              {/* Código */}
              <div className="flex items-center gap-2 sm:flex-col sm:items-start sm:gap-1 sm:min-w-20">
