...
import {
  ...
  Filter,
  Search,
  ChevronsUpDown, // Icon for collapsible trigger
...
const MonitorLecturas = () => {
  const { periodos, sectores, claves, isLoading, error, activePeriodoId } =
    useMonitor();

  const pageBreadcrumbs = [{ label: 'Monitor' }, { label: 'Lecturas' }];
  ...
  //Nombre del header
  const headerTitle = 'Monitor de Lecturas';
  useEffect(() => {
...
export default MonitorLecturas;
