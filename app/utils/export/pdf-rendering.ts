...
export function renderKPISection(
  ...
  for (let index = 0; index < kpis.length; index++) {
    ...
    doc.setFontSize(9);
    doc.setTextColor(100, 100, 100);
    doc.text(kpi.label, xPosition + kpiWidth / 2, currentY + 5, {
      align: 'center'
    });

    doc.setFontSize(12);
    doc.setTextColor(0, 0, 0);
    doc.text(String(kpi.value), xPosition + kpiWidth / 2, currentY + 12, {
      align: 'center'
    });
...
export function renderTableSection(
  ...
  const tableData = section.data.map((row: any) => {
    return section.columns!.map((col: any) => {
      const value = row[col.key];
      return col.formatter ? col.formatter(value) : String(value || '');
    });
  });

  const headers = section.columns.map((col: any) => col.header);
  const columnStyles: Record<number, any> = {};

  for (const [index, col] of section.columns.entries()) {
    columnStyles[index] = {
      halign: (col as any).align || 'left',
      cellWidth: (col as any).width || 'auto'
    };
  }
...
export function renderSection(
  ...
  if ((section as any).title) {
    doc.setFontSize(14);
    doc.setTextColor(0, 0, 0);
    doc.text((section as any).title, margin, currentY);
    currentY += 8;
  }

  switch (section.type) {
    case 'kpis':
      if ((section as any).kpis && (section as any).kpis.length > 0) {
        currentY = renderKPISection(
          doc,
          (section as any).kpis,
          margin,
          contentWidth,
    ...
    case 'table':
      if (section.content && (section as any).columns) {
