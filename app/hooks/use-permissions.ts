...
export function usePermissions(route: string) {
  ...
  return {
    ...
    hasAnyPermission: canView(route) || canCreate(route) || canEdit(route) || canDelete(route),
...
export function useCurrentRoutePermissions() {
  ...
  // En React Router v7, podemos usar window.location.pathname
  ...
  return {
    ...
    hasAnyPermission: canView(currentRoute) || canCreate(currentRoute) || canEdit(currentRoute) || canDelete(currentRoute),
    isLoading: permissionsLoading
  };
}
