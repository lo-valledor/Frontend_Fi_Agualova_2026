  ...
  const filteredContracts = useMemo(() => {
    return contracts.filter((contract) => {
      ...
      if (
        ...
        contract.cicloFacturacion !== filters.cicloFacturacion
      ) {
        return false;
      }
      ...
      if (
        filters.activo &&
        filters.activo !== 'all' &&
        contract.activo.toString() !== filters.activo
      ) {
        return false;
      }
      ...
      if (
        ...
        contract.liberadoCorte.toString() !== filters.liberadoCorte
      ) {
        return false;
      }
      ...
      if (filters.fechaTerminoDesde || filters.fechaTerminoHasta) {
        const fechaTermino = contract.fechaTermino
          ? new Date(contract.fechaTermino)
          : null;

        // Si no hay fecha de término y se está filtrando por fechas, excluir
        if (
          !fechaTermino &&
          (filters.fechaTerminoDesde || filters.fechaTerminoHasta)
        ) {
          return false;
        }
