...
// REFACTOR: Extraer MetricCard como componente independiente
...
// ✅ REFACTOR: Extraer HistoryCard como componente independiente
  ...
  <div className='p-3 bg-muted/30 rounded-xl border border-border/20'>
    <div className='flex items-center justify-between mb-2'>
      <div className='flex items-center gap-2'>
        ...
        <span className='font-mono text-sm font-medium'>{item.LM_Periodo}</span>
      </div>
    ...
    {Boolean(item.LM_ValorLecturaActual) && (
...
export default function AnalisisConsumo({
  ...
  const getMensualComparisonData = useMemo(() => {
    ...
    // ✅ REFACTOR: Usar for...of en lugar de forEach
  ...
  // ✅ REFACTOR: Calcular valores para tendencias y colores
  let trendDirection: 'up' | 'down' | 'neutral' = 'neutral';
  let trendColor: 'blue' | 'green' | 'amber' | 'purple' | 'red' = 'purple';
  let trendText: string | undefined;

  if (variacionMesAnterior !== null) {
    if (variacionMesAnterior > 0) {
      trendDirection = 'up';
      trendColor = 'red';
      trendText = `Consumo ${Math.abs(variacionMesAnterior).toFixed(1)}% mayor que el mes anterior`;
    } else {
      trendDirection = 'down';
      trendColor = 'green';
      trendText = `Consumo ${Math.abs(variacionMesAnterior).toFixed(1)}% menor que el mes anterior`;
    }
  } // ✅ REFACTOR: Verificar condiciones de renderizado
  const hasValidData = tieneDatosValidos(dataEtapaCuatro);
  const hasError = Boolean(error);
  const hasComparisionData = getMensualComparisonData.some(
    item => item.consumoActual !== null && item.consumoAnterior !== null
  );

  // ✅ REFACTOR: Renderizar contenido según estado
  const renderContent = () => {
    if (hasError) {
      return (
            ...
            {estadisticas ? (
              ...
              <div className='p-6 text-center'>
                ...
                <p className='text-xs text-muted-foreground'>
                  Se requieren lecturas válidas con fechas y consumos mayores a
                  0
                </p>
              </div>
            )}
                ...
                <div className='grid grid-cols-2 gap-2 text-xs'>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      {estadisticas.consumoTotal.toLocaleString('es-CL')} kWh
                    </span>
                  </div>
                  <div className='flex justify-between'>
            ...
            <div className='hidden md:block space-y-6'>
              ...
              <div className='bg-background/95 rounded-xl border border-border/40 overflow-hidden'>
                ...
                <div className='p-4'>
                  ...
                  >
                    <LineChart
                      data={getDatosFiltrados(dataEtapaCuatro).toReversed()}
                      margin={{
                        top: 20,
                        right: 30,
                        left: 20,
          ...
          <div className='space-y-3'>
            <div className='flex items-center justify-between mb-3'>
              <div className='text-xs text-muted-foreground'>
                Últimas {Math.min(datosValidosOrdenados.length, 10)} lecturas
                registradas
              </div>
            ...
            {getDatosFiltrados(dataEtapaCuatro)
              ...
              .map(item => (
                <HistoryCard
                  key={`${item.LM_Periodo}-${item.LM_FechaLectura}`}
                  item={item}
                />
              ))}
                  ...
                  <Table>
                    <TableHeader>
                      <TableRow>
                        ...
                        <TableHead className='text-xs'>Fecha Lectura</TableHead>
                    ...
                    <TableBody>
                      {datosValidosOrdenados.toReversed().map(item => (
                        <TableRow
                          key={`${item.LM_Periodo}-${item.LM_FechaLectura}`}
                        >
                          ...
                          <TableCell className='text-xs text-right font-semibold'>
                            {item.LM_ConsumoPeriodo?.toLocaleString('es-CL')}
                          </TableCell>
                ...
                <div className='space-y-2 text-xs'>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      ...
                      {estadisticas?.consumoMaximo.toLocaleString('es-CL')} kWh)
                    </span>
                  </div>
                  <div className='flex justify-between'>
                    ...
                    <span className='font-medium'>
                      ...
                      {estadisticas?.consumoMinimo.toLocaleString('es-CL')} kWh)
                    </span>
                  </div>
                  <div className='flex justify-between'>
            ...
            <div className='p-4 flex-1 overflow-auto'>
              <Table>
                ...
                <TableBody>
                  {datosValidosOrdenados.toReversed().map(item => (
                    <TableRow
                      key={`sheet-${item.LM_Periodo}-${item.LM_FechaLectura}`}
                    >
